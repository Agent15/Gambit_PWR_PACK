...
namespace Gambonanza.MysterysGambit
{
    ...
    /// 
    /// WIP: If one of this gambit's sub-gambits normally displays a sprite on top of it (Prayer, Violet, etc)
    /// It tends to throw game-breaking exceptions. I'm still looking for the cause of this error.
    /// </summary>
    public sealed class GambitMystery : BaseGambit
    {
        ...
        private GambitBehaviour[] children = new GambitBehaviour[3];
        ...
        private void Start()
        {
                ...
                for (int x = 0; x < children.Length; x++)
                {
                    ...
                    s += children[x].Info.GambitName + '\n';//DEBUG
                }
                PromotionManager.Instance.OnPromotePlayer += ShowcaseMode;//Shhhhh
        ...
        private void OnDestroy()
        {
            ...
            PromotionManager.Instance.OnPromotePlayer -= ShowcaseMode;//Nothing to see here ;)
        }
        ...
        {
            BaseGambit[] children = FindObjectsByType<BaseGambit>();
        ...
        public GambitBehaviour CreateGambitInstance(Type childType = null)
        /*{
            ...
            GambitBehaviour behavior = host.GetComponent<GambitBehaviour>();
        ...
        {
			SO_Gambit randomUnpossessedGambit = SingletonMonoBehaviour<GambitLibrary>.Instance.GetRandomUnpossessedGambit("MYSTERY");
            Vector3 zero = new Vector3(x: 0, y:0, z:0);
			GambitBehaviour gambitBehaviour = Instantiate<GambitBehaviour>(SingletonMonoBehaviour<GambitLibrary>.Instance.Gambits[SingletonMonoBehaviour<GambitLibrary>.Instance.GambitsInfo.IndexOf(randomUnpossessedGambit)], zero, Quaternion.identity);
			gambitBehaviour.transform.localScale = Vector3.zero;

            // Maybe try this later
            // BindingFlags flags = BindingFlags.NonPublic | BindingFlags.Instance;
            // FieldInfo gambitField = typeof(BaseGambit).GetField("m_Gambit", flags);
            // gambitField.GetValue(gambitBehaviour);
            return gambitBehaviour;
        }
        // When I make a showcase for this mod, I want to know what child gambits I'm working with
        private void ShowcaseMode(PieceType type)
        {
            try{
            if (type == PieceType.PAWN && ChessDataManager.Instance.Coins == 420)
            {
                for (int x = 0; x < children.Length; x++)
                {
                    if (children[x] is not null) Destroy(children[x].gameObject);
                }
